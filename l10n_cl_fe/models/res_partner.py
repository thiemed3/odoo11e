...
class ResPartner(models.Model):
    ...
    def onchange_document(self):
        ...
        if self.document_number and ((
            ...
            if exist:
                # En este caso, el numero de documento esta siendo utilizado por otro contacto
                # Entonces se debe quitar el valor de document_number y vat para que no esten
                # duplicados. Y se envia un mensaje al usuario para que sepa lo que esta pasando
                # Pero cundo se utiliza el boton de actualizar RUT, el mensaje al usuario no
                # se muestra, por lo tanto dejo un mensaje en el log de Odoo
                _logger.warning("El contacto %s está utilizando este numero de documento" %(exist.name))
                self.vat = ''
            ...
            try:
                if otro[:1] == '0':
                    otro=otro[1:]
                url = "http://api.konos.cl:8000/api_rut_sii/index.php?id=%s" % otro
            ...
            if response and response.status_code!=200:
                ...
                if len(vals)>40:
                    ...
                    # 2. Para homologar la comuna, se creo una funcion que encapsule las validaciones
                    busca_comuna = self._verificar_comuna(vals)
                    self.activity_description = self._buscar_giro(self.find_between_r( vals, "[rubro] => ", ")" ).rstrip())
                    #-#_logger.warning("** * *** Direccion *%s* - Tamaño: %s " %(csx_dir, len(csx_dir)))
                    #-#_logger.warning("** * *** Comuna %s" %(busca_comuna))

                    # 3. Verifico si las variables vienen vacia, para no reemplazar la data que esta 
                    # almacenada. Estaba ocurriendo que si la API regresa un valor en blanco, y la
                    # ficha del contacto tienen información, la misma es reemplazada por blanco
                    if len(csx_nombre) > 0:
                    ...
                    # En la data solo hay empresas jurídicas. De todas maneras por defecto usaremos empresas ya que es lo común
                    self.company_type="company"
                    # Va contra len > 1 porque aunque no se encuentra direccion, en la concatenación
                    # de las variables, se agrega un espación en blanco
                    if len(csx_dir) > 1:
                        self.street = csx_dir
                    if len(csx_dir2) > 1:
    ...
    def _verificar_comuna(self, registro):
        comuna = self.find_between_r( registro, "[comuna] => ", "[region]" ).rstrip().lstrip()
        ...
        _logger.warning("Comuna a ser buscada: (%s) - Se encontro: %s" %(comuna.title(), busca_comuna))
    ...
    def _buscar_giro(self, registro):
        ...
        if busca_giro:
            ...
            _logger.warning("Giro a ser buscado: (%s) - Se encontro: %s" %(giro, busca_giro))
            ## # -----------------------------------------
